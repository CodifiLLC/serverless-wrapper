...
function getConfiguration(obj, cwd, existingConfig) {
	...
	const objCors = {
		origins: obj?.cors?.origins ?? [],
		methods: obj?.cors?.methods ?? [],
		headers: obj?.cors?.headers ?? [],
	};
	const existingCors = {
		...
		.map(packageObj => {
			...
			return Object.values(packageObj.functions)
				.map(funcObj => {
					...
					const basePath = path.join(cwd, packageDirectory, packageObj.name);
					return {
						envKeys,
						envMap,
						cors,
						...
						path: path.join(basePath, funcObj.entry ? path.dirname(funcObj.entry) : funcObj.name),
						indexPath: path.join(basePath, funcObj.entry ?? path.join(funcObj.name, 'index.mjs')),
