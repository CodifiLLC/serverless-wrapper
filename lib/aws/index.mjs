...
async function prepNodeModules() {
	...
	await clean({ directory: libPath, globs: ['**/*.d.ts', '**/*.d.mts'], noDefaults: false });
	...
	await clean({ directory: BUILD_FOLDER, globs: ['**/*.d.ts', '**/*.d.mts'], noDefaults: false });
}
...
const runPulumiDeploy = async function ({environment}) {
    ...
    const pulumiProgram = async () => {
		const iamForLambda = new aws.iam.Role("iam_for_lambda", {
			name: `iam_for_lambda${process.env.npm_package_name}`,
	...
	for (const func of functionsToBuild) {
		...
		*/
		...
		packageObj.functions.push({
			...
			binary: false,
			path: `./app/${func.packageName}/${func.functionName}/${func.indexPath.endsWith('index.mjs') ? path.basename(func.indexPath) : func.functionName + '.mjs'}`,
			// main: 'do-wrapper.mjs',
		...
		if (func.indexPath.endsWith('index.mjs')) {
			const funcFiles = walk(func.path);
			funcFiles.forEach(entry => {
			...
			funcFileMap[func.indexPath] = `app/${func.packageName}/${func.functionName}/${func.functionName}.mjs`;
		}
		// console.log(JSON.stringify(projObj, null, 2))
		// console.log('file map', JSON.stringify(funcFileMap, null, 2))
	}
...
// 		Object.keys(libFileMap).forEach(file => {
// 			zip.file(file, { name: `lib/${libFileMap[file]}` });
// 		});
...
// console.log('\n\n\nlib', libFileMap, '\n\n\n\n\n\n');
		...
		const mwName = filePath => 'mw_' + path.basename(filePath).replace(/[^a-z0-9]/g, '_');
		projObj.packages.map(p => p.functions.map(f => ({
				pattern: `/${p.name}/${f.name}`,
				funcName: `${p.name}_${f.name}`.replace(/[^a-z0-9]/g, '_'),
				...f
			})))
			.flat()
			.forEach(f => {
				functionImports.push(`import { main as ${f.funcName} } from '${f.path}';`)
				functionMap[f.pattern] = `{ func: ${f.funcName}, authMiddleware: [${f.authMiddleware.map(mwName).join(',')}], otherMiddleware: [${f.otherMiddleware.map(mwName).join(',')}] }`;
				f.authMiddleware.forEach(a => {
					middlewareList[a] = mwName(a);
				});
				f.otherMiddleware.forEach(a => {
					middlewareList[a] = mwName(a);
				});
			});
		Object.entries(middlewareList).forEach(([k, v]) => {
			functionImports.push(`import ${v} from '${k}';`);
		});
		...
		zip.append(
			fs.readFileSync(path.join(awsPkgDir, 'aws-wrapper.mjs'), 'utf8')
				.toString()
				.replace('/**{IMPORTS}**/', functionImports.join('\n'))
				.replace("{ PATH: 'FUNCTION_CONFIG' }", '{\n' + Object.entries(functionMap).map(([k,v]) => `'${k}': ${v}`).join(',\n') + '}'),
			{name: 'index.mjs'}
		);
		zip.file(
