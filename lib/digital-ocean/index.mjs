...
const cleanParams = { globs: ['**/*.d.ts', '**/*.d.mts'], noDefaults: false };

const functionData = getData();

const BUILD_FOLDER = path.join(process.env.BUILD_TMP_DIR ?? './dist', 'do-build');
...
function invokeDoctlDeploy(path, extra = '') {
	return invokeDoctlCommand(`serverless deploy ${path} ${extra}`)
...
async function prepNodeModules() {
	...
	await clean({ ...cleanParams, directory: libPath });
	...
	await clean({ ...cleanParams, directory: BUILD_FOLDER });
}
...
if (!isDeployOnly) {
	...
	for (const func of functionsToBuild) {
		...
		await buildZip(funcRootPath, zip => {
			...
			const mwFuncName = f => path.basename(f).replace('.mjs', '').replace(/[^a-z0-9]/g);
			const importStr = mappedMiddleware
								.filter(Boolean)
								.map(m => `import ${mwFuncName(m.newPath)} from './${m.newPath}';`)
								.join('\n');
			//TODO weirdly named files??
			zip.append(
				fs.readFileSync(path.join(doPkgDir, 'do-wrapper.mjs'), 'utf8')
					.toString()
					.replace('const AUTH_MIDDLEWARE = []', `const AUTH_MIDDLEWARE = [${authMiddleware.map(a => mwFuncName(a.newPath)).join(",")}]`)
					.replace('const OTHER_MIDDLEWARE = []', `const OTHER_MIDDLEWARE = [${otherMiddleware.map(a => mwFuncName(a.newPath)).join(",")}]`)
					.replace('/** MW_IMPORTS **/', importStr)
			...
			zipNodeModules(BUILD_FOLDER, '', (packageStr) => {
				...
				const packageObj = packageStr ? JSON.parse(packageStr) : {name: 'default-do', version: '0.1'};
...
if (functionsToDeploy.length) {
	for (const func of functionsToDeploy) {
		await invokeDoctlDeploy(DEPLOY_FOLDER, `--include ${func}`);
	}
}
