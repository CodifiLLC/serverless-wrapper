...
import {getData} from "./package-parser.mjs";
...
for (const packageObj of functionData.functions) {
	...
	fs.readdirSync('./lib')
		...
		.forEach(f => {
			fs.cpSync(path.join('./lib', f), path.join(DEPLOY_FOLDER, f), {recursive: true});
		});
	fs.writeFileSync(
		'./deploy/index.cjs',
		fs.readFileSync('./openwhisk-wrapper.cjs', 'utf8').replace(/const KNOWN_KEYS = \[\]/,`const KNOWN_KEYS = ['${functionData.envKeys.join("', '")}']`)
	);
